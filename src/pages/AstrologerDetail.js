...
function AstrologerDetail() {
  ...
  const [details, setDetails] = useState(parse);
  const [Reviewlist, setReviewlists] = useState([]);
  ...
  const reviewlist = () => {
    const item = {
      astrologer_id: details?._id,
    };
    axios
      .post(
        "http://103.104.74.215:3012/api/astrologer/astrologer_review_list",
        item
      )
      .then((res) => {
        setReviewlists(res.data.data);
      });
  };
  ...
  const tokenGene = (audiorate) => {
    let totalminute = walletAmnt / audiorate;
    let finltime = Math.floor(totalminute);
    localStorage.setItem("totalminute", finltime);
    const assadas = localStorage.getItem("vcidddata");
                  ...
                  >
                    ...
                    >
                      <div
                        onClick={() => {
                          {
                            walletAmnt >= list.audio_rate
                              ? tokenGene(list.audio_rate)
                              : alert("You have Insufficient balance");
                          }
                          localStorage.setItem("vcdata", JSON.stringify(list));
                          localStorage.setItem("vcidddata", list._id);
                          localStorage.setItem("videorate", list.audio);
                          {
                        ...
                        onClick={() => {
                          {
                            walletAmnt >= list.video_rate
                              ? tokenGen(list.video_rate)
                              : alert("You have Insufficient balance");
                          }
                          localStorage.setItem("vcdata", JSON.stringify(list));
                          localStorage.setItem("vcidddata", list._id);
                          localStorage.setItem("videorate", list.video_rate);
                          {
                      ...
                      >
                        <h4 style={{ color: "white" }}>Video Call</h4>
                                    ...
                                    <div class="people-box">
                                      <div>
                                        <div class="people-image">
                                          ...
                                          {review.user_data.user_pic ? ( <img
                                            src={
                                              "http://103.104.74.215:3012/uploads/" +
                                              review?.user_data.user_pic
                                            }
                                            class="img-fluid blur-up lazyload"
                                            alt=""
                                          />):( <img
